...
interface IncidenciaDialogProps {
  ...
  onSubmit: (data: Omit<IncidenciaTable, "id">) => void
  defaultValues?: Partial<IncidenciaTable>
}

export function IncidenciaDialog({ open, onOpenChange, onSubmit, defaultValues }: IncidenciaDialogProps) {
  ...
  const gradosDict = Object.fromEntries(
    grados.map((item) => [item.nombre, item.cantidadPuntos])
  )
  ...
  function handleSubmit(values: z.infer<typeof formSchema>) {
    const data = {
      ...
      // puntos: values.severidad === "Leve" ? gradosDict["Leve"] : values.severidad === "Grave" ? gradosDict["Grave"] : gradosDict["Muy Grave"],
    }
    onSubmit(data)
    form.reset()
    onOpenChange(false)
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent>
                      <SelectItem value="Leve">Leve ({`${gradosDict["Leve"]}`})</SelectItem>
                      <SelectItem value="Grave">Grave ({`${gradosDict["Grave"]}`})</SelectItem>
                      <SelectItem value="Muy Grave">Muy Grave ({`${gradosDict["Muy Grave"]}`})</SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage />
                </FormItem>
